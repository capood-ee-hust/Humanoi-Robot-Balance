...
def selectRobot( vel, step_len, spno):
    ...
    robotpath = os.path.join(dirname, 'Unitree/g1.xml') #model unitree G1
...
class myRobot:
    ...
    def COMcontrol(self, model, data):
        self.ocm_des = np.array([self.oCMx(data.time), self.oCMy(data.time), self.oCMz(data.time)])  
